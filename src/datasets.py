...
def transform_embeddings(transform_type, data):
    if data.ndim == 3:
        N, pairs, features = data.shape
        emb_a = data[:, 0, :].contiguous() # (N, 10)
        emb_b = data[:, 1, :].contiguous() # (N, 10)
        emb_a = torch.Tensor(Normalizer().fit_transform(emb_a))
        emb_b = torch.Tensor(Normalizer().transform(emb_b))
        return torch.stack([emb_a, emb_b], dim=1) # (N, 2, 10)
