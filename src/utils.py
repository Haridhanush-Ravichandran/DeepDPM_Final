...
# Remap to contiguous zero-based indices
...
def best_cluster_fit(y_true, y_pred):
    ...
    print(f"y_true shape: {y_true.shape}, min: {y_true.min()}, max: {y_true.max()}, unique: {len(np.unique(y_true))}")
    print(f"y_pred shape: {y_pred.shape}, min: {y_pred.min()}, max: {y_pred.max()}, unique: {len(np.unique(y_pred))}")
